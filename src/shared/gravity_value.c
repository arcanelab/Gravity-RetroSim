...
#pragma unused(table)
    ...
    if (VALUE_ISA_CLOSURE(value))
        value = VALUE_FROM_OBJECT(VALUE_AS_CLOSURE(value)->f);

    if (VALUE_ISA_FUNCTION(value))
    {
        gravity_function_t *f = VALUE_AS_FUNCTION(value);
        if (f->tag == EXEC_TYPE_SPECIAL)
            gravity_function_special_serialize(f, VALUE_AS_CSTRING(key), json);
        else
        {
...
gravity_class_t *gravity_class_get_meta(gravity_class_t *c)
{
    ...
    if (c->objclass == gravity_class_object)
        return c;
    return c->objclass;
}
...
void gravity_class_serialize(gravity_class_t *c, json_t *json)
{
    ...
    if ((c != meta) && (meta->nivars > 0))
        json_add_int(json, GRAVITY_JSON_LABELSIVAR, meta->nivars);
...
gravity_class_t *gravity_class_deserialize(gravity_vm *vm, json_value *json)
{
    ...
    if (string_casencmp(key, GRAVITY_JSON_LABELIDENTIFIER, strlen(key)) != 0)
        return NULL;
    ...
    { // from 2 to skip type and identifier
...
static void gravity_class_free_internal(gravity_vm *vm, gravity_class_t *c, bool skip_base)
{
    ...
    if (c->xdata && vm)
    {
        ...
        if (delegate->bridge_free)
            delegate->bridge_free(vm, (gravity_object_t *)c);
    }
...
gravity_class_t *gravity_class_lookup_class_identifier(gravity_class_t *c, const char *identifier)
{
    while (c)
    {
        if (string_cmp(c->identifier, identifier) == 0)
            return c;
        c = c->superclass;
    }
    return NULL;
            ...
            for (uint32_t j = 0; j < m; ++j)
            {
                ...
                case json_array:
                {
                    ...
                    for (uint32_t k = 0; k < count; ++k)
                    {
                        ...
                        {
                        case json_integer:
                            v = VALUE_FROM_INT((gravity_int_t)jsonv->u.integer);
                            break;
                        case json_double:
                            v = VALUE_FROM_FLOAT((gravity_float_t)jsonv->u.dbl);
                            break;
                        case json_boolean:
                            v = VALUE_FROM_BOOL(jsonv->u.boolean);
                            break;
                        case json_string:
                            v = VALUE_FROM_STRING(vm, jsonv->u.string.ptr, jsonv->u.string.length);
                            break;
                        default:
                            goto abort_load;
                        }
...
void gravity_function_free(gravity_vm *vm, gravity_function_t *f)
{
    ...
    if (f->xdata && vm)
    {
        gravity_delegate_t *delegate = gravity_vm_delegate(vm);
        if (delegate->bridge_free)
            delegate->bridge_free(vm, (gravity_object_t *)f);
    }
...
uint32_t gravity_function_size(gravity_vm *vm, gravity_function_t *f)
{
    ...
    {
        if (f->bytecode)
            func_size += f->ninsts * sizeof(uint32_t);
        // cpool size
        size_t n = marray_size(f->cpool);
        for (size_t i = 0; i < n; i++)
        {
...
void gravity_function_blacken(gravity_vm *vm, gravity_function_t *f)
{
    ...
    {
        if (f->special[0])
            gravity_gray_object(vm, (gravity_object_t *)f->special[0]);
        if (f->special[1])
            gravity_gray_object(vm, (gravity_object_t *)f->special[1]);
    }
...
{
    if ((!obj) || (!OBJECT_IS_VALID(obj)))
        return "";

    if (OBJECT_ISA_INT(obj))
        return "INT";
    if (OBJECT_ISA_FLOAT(obj))
        return "FLOAT";
    if (OBJECT_ISA_BOOL(obj))
        return "BOOL";
...
void gravity_object_blacken(gravity_vm *vm, gravity_object_t *obj)
{
    ...
    if (obj->gc.blacken)
        ...
        gravity_list_blacken(vm, (gravity_list_t *)obj);
    else if (OBJECT_ISA_MAP(obj))
...
gravity_instance_t *gravity_instance_new(gravity_vm *vm, gravity_class_t *c)
{
    ...
    if (c->nivars)
        instance->ivars = (gravity_value_t *)mem_alloc(NULL, c->nivars * sizeof(gravity_value_t));
    for (uint32_t i = 0; i < c->nivars; ++i)
...
void gravity_instance_deinit(gravity_vm *vm, gravity_instance_t *i)
{
    ...
    if (closure)
        gravity_vm_runclosure(vm, closure, VALUE_FROM_OBJECT(i), NULL, 0);
}
...
void gravity_instance_free(gravity_vm *vm, gravity_instance_t *i)
{
    ...
    if (i->xdata && vm)
    {
        gravity_delegate_t *delegate = gravity_vm_delegate(vm);
        if (delegate->bridge_free)
            delegate->bridge_free(vm, (gravity_object_t *)i);
    }
...
gravity_value_t gravity_instance_lookup_property(gravity_vm *vm, gravity_instance_t *i, gravity_value_t key)
{
    ...
    if (!func || func->tag != EXEC_TYPE_SPECIAL)
        return VALUE_NOT_VALID;
    ...
    if (FUNCTION_ISA_GETTER(func))
    {
        ...
        if (gravity_vm_runclosure(vm, getter, VALUE_FROM_NULL, NULL, 0))
...
bool gravity_value_equals(gravity_value_t v1, gravity_value_t v2)
{
    ...
    else if (v1.isa == gravity_class_list)
    {
        ...
        if (marray_size(list1->array) != marray_size(list2->array))
            return false;
...
{
    if ((v.isa == gravity_class_class) && (v.p->objclass == gravity_class_object))
        return (gravity_class_t *)v.p;
    if ((v.isa == gravity_class_instance) || (v.isa == gravity_class_class))
        return (v.p) ? v.p->objclass : NULL;
    return v.isa;
}
...
void gravity_value_serialize(const char *key, gravity_value_t v, json_t *json)
{
    ...
    if (VALUE_ISA_MAP(v))
    {
        ...
        json_begin_object(json, key);
        if (!json_option_isset(json, json_opt_no_maptype))
...
bool gravity_value_isobject(gravity_value_t v)
{
    ...
    if ((v.isa == NULL) || (v.isa == gravity_class_int) || (v.isa == gravity_class_float) ||
        (v.isa == gravity_class_bool) || (v.isa == gravity_class_null) || (v.p == NULL))
        return false;
    ...
    if ((v.isa == gravity_class_string) || (v.isa == gravity_class_object) || (v.isa == gravity_class_function) ||
        ...
        (v.isa == gravity_class_map) || (v.isa == gravity_class_range) || (v.isa == gravity_class_upvalue))
        return true;
...
gravity_list_t *gravity_list_from_array(gravity_vm *vm, uint32_t n, gravity_value_t *p)
{
    ...
    for (size_t i = 0; i < n; ++i)
        marray_push(gravity_value_t, list->array, p[i]);
...
static gravity_map_t *gravity_map_deserialize(gravity_vm *vm, json_value *json)
{
    ...
    { // from 1 to skip type
        ...
        {
        case json_integer:
            value = VALUE_FROM_INT((gravity_int_t)jsonv->u.integer);
            break;
        case json_double:
            value = VALUE_FROM_FLOAT((gravity_float_t)jsonv->u.dbl);
            break;
        case json_boolean:
            value = VALUE_FROM_BOOL(jsonv->u.boolean);
            break;
        case json_string:
            value = VALUE_FROM_STRING(vm, jsonv->u.string.ptr, jsonv->u.string.length);
            break;
        case json_null:
